...
public class Activity_Direccion extends AppCompatActivity {
    ...
    final Map<String, Object> textos_directivo = new HashMap<>();
    private Uri selectedImageUri;
    private List<PublicacionDatosPersonal> lista_datos_directivos;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        FloatingActionButton boton_direccion = findViewById(R.id.boton_direccion);

        String rol = Menu.getRol();

        // Verificar si el rol es "admin" para mostrar u ocultar el botón flotante
        if (rol != null && rol.equals("admin")) {
            boton_direccion.setVisibility(View.VISIBLE);
            boton_direccion.setOnClickListener(new View.OnClickListener() {
                @Override
            ...
            boton_direccion.setVisibility(View.GONE);
        }

        lista_datos_directivos = new ArrayList<>();
        ...
        db.collection("Personal_Academico")
                ...
                .addOnSuccessListener(queryDocumentSnapshots -> {
                    for (QueryDocumentSnapshot document : queryDocumentSnapshots) {
                        ...
                        lista_datos_directivos.add(publicacion);
        ...
        mRecyclerView =findViewById(R.id.recycle_direccion);
        mAdapter = new PublicacionPersonalAdapter(Activity_Direccion.this, lista_datos_directivos);
        mRecyclerView.setLayoutManager(new LinearLayoutManager(Activity_Direccion.this));
    ...
    public void HacerPublicacion(){
        ...
        // Crea el AlertDialog
        AlertDialog.Builder builder = new AlertDialog.Builder(Activity_Direccion.this);
        ...
        builder.setPositiveButton("Aceptar", (dialog, which) -> {
            ...
            if (!newNombre.isEmpty()) {
                textos_directivo.put("nombre", newNombre);
            }
            if (!newCargo.isEmpty()) {
                textos_directivo.put("cargo", newCargo);
            }
            if (!newCorreo.isEmpty()) {
                textos_directivo.put("correo", newCorreo);
            }
            if (!newTelefono.isEmpty()) {
                textos_directivo.put("telefono", newTelefono);
            }
            if (!newExtension.isEmpty()) {
                textos_directivo.put("extension", newExtension);
            }
            if (!newUbicacion.isEmpty()) {
                textos_directivo.put("ubicacion", newUbicacion);
            }

            if(selectedImageUri!=null) {
                // Obtén una referencia a la ubicación donde se guardará la imagen en Storage
                StorageReference imagenRef = storageRef.child("imagenes_personal/imagenes_directivo" + UUID.randomUUID().toString());
                ...
                imagenRef.putFile(selectedImageUri)
                        .addOnSuccessListener(taskSnapshot -> {
                            ...
                            imagenRef.getDownloadUrl().addOnSuccessListener(uri -> {
                                ...
                                textos_directivo.put("url_imagen", imageUrl);
                                ...
                                publicacionesRef.add(textos_directivo)
                                        .addOnSuccessListener(documentReference -> {
                                            ...
                                            lista_datos_directivos.add(nuevaPublicacion);

                                            // Notificar al adaptador sobre el cambio en los datos
                                            mAdapter.notifyDataSetChanged();

                                            // Mostrar un mensaje de éxito
                                            Toast.makeText(Activity_Direccion.this, "Publicación agregada correctamente", Toast.LENGTH_SHORT).show();
                                        })
                                        .addOnFailureListener(e -> {
                                            // Mostrar un mensaje de error en caso de falla
                                            Toast.makeText(Activity_Direccion.this, "Error al agregar la publicación", Toast.LENGTH_SHORT).show();
