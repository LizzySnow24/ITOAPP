...
public class Activity_Planeacion extends AppCompatActivity {
    private static final int PICK_IMAGE_REQUEST = 1;
    private AlertDialog alertDialog;
    private FirebaseFirestore db = FirebaseFirestore.getInstance();
    private FirebaseStorage storage = FirebaseStorage.getInstance();
    private StorageReference storageRef = storage.getReference();
    final Map<String, Object> textos_planeacion = new HashMap<>();
    private Uri selectedImageUri;
    private List<PublicacionDatosPersonal> lista_datos_planeacion;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        FloatingActionButton botonCrearPlaneacion = findViewById(R.id.botonCrearPlaneacion);

        String rol = Menu.getRol();

        // Verificar si el rol es "admin" para mostrar u ocultar el botón flotante
        if (rol != null && rol.equals("admin")) {
            botonCrearPlaneacion.setVisibility(View.VISIBLE);
            botonCrearPlaneacion.setOnClickListener(new View.OnClickListener() {
                @Override
            ...
            botonCrearPlaneacion.setVisibility(View.GONE);
        }

        lista_datos_planeacion = new ArrayList<>();
        ...
        db.collection("Personal_Academico")
                ...
                .addOnSuccessListener(queryDocumentSnapshots -> {
                    for (QueryDocumentSnapshot document : queryDocumentSnapshots) {
                        ...
                        lista_datos_planeacion.add(publicacion);
        ...
        mRecyclerView =findViewById(R.id.recyclePlaneacion);
        mAdapter = new PublicacionPersonalAdapter(Activity_Planeacion.this, lista_datos_planeacion);
        mRecyclerView.setLayoutManager(new LinearLayoutManager(Activity_Planeacion.this));
    ...
    public void HacerPublicacion(){
        ...
        // Crea el AlertDialog
        AlertDialog.Builder builder = new AlertDialog.Builder(Activity_Planeacion.this);
        ...
        builder.setPositiveButton("Aceptar", (dialog, which) -> {
            ...
            if (!newNombre.isEmpty()) {
                textos_planeacion.put("nombre", newNombre);
            }
            if (!newCargo.isEmpty()) {
                textos_planeacion.put("cargo", newCargo);
            }
            if (!newCorreo.isEmpty()) {
                textos_planeacion.put("correo", newCorreo);
            }
            if (!newTelefono.isEmpty()) {
                textos_planeacion.put("telefono", newTelefono);
            }
            if (!newExtension.isEmpty()) {
                textos_planeacion.put("extension", newExtension);
            }
